...
import ete3
import warnings
from typing import Dict, Callable, Optional, Set, Sequence, Mapping
...
def _add_isotype_to_mp_trees(
    ...
    """Adds isotypes to leaves of MP trees from phylip_parse.parse_outfile, and
    when a leaf corresponds to multiple observed isotypes, the leaf is split
    into two sister leaves (without adding a new node to the tree), each with
    its own observed isotype and the correct corresponding abundance."""
    ...
    if isotypemap is None and isotypemap_file is None:
        ...
        idmap, isotypemap = _process_isotype_arguments(
        ...
        for tree in trees:
            ...
            for leaf in tree.iter_leaves():
                this_leaf_isotypes = list(
                    (isoname, len(og_id_set))
                    for isoname, og_id_set in newidmap[leaf.name].items()
                )
