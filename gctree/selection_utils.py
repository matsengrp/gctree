...
def lambda_selection(node, tree, targetAAseqs, hd2affy, A_total, B_total, Lp):
    ...
    def calc_binding_time(Kd_n, A_total, B_total):
        ...
        obj_min = minimize(
            obj, A_total, bounds=[[1e-10, A_total]], method="L-BFGS-B", tol=1e-20
        )
...
def find_A_total(carry_cap, B_total, f_full, mature_affy, U):
    def A_total_fun(A, B_total, Kd_n):
        return A + scipy.sum(B_total / (1 + Kd_n / A))

    def C_A(A, A_total, f_full, U):
        return U * (A_total - A) / f_full

    def A_obj(carry_cap, B_total, f_full, Kd_n, U):
        def obj(A):
            return (carry_cap - C_A(A, A_total_fun(A, B_total, Kd_n), f_full, U)) ** 2
    ...
    obj_min = minimize(
        obj, 1e-20, bounds=[[1e-20, carry_cap]], method="L-BFGS-B", tol=1e-20
    )
...
def find_Lp(f_full, U):
    ...
    try:
        ...
        def obj_T_A(p):
            return solve_T_BA_low_epsilon(p, f_full, U)
...
def plot_runstats(runstats, outbase, colors):
    ...
    for k in list(range(*bounds)):
        ...
        ax.plot(
            t,
            scipy.array([r[0][k] for r in runstats]),
            lw=2,
            color=color,
            label="Dist {}".format(k),
        )
