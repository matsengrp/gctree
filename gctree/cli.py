...
def test(args):
    ...
    for p in ps:
        for q in qs:
            ...
            for tree_hash in tree_dict:
                df.loc[ct] = (p, q, f'p={p}, q={q}', tree_dict[tree_hash][2],
                              np.exp(tree_dict[tree_hash][1]))
                ct += 1
    print()
    ...
    g = sns.lmplot(x='L', y='f', col='p', row='q', hue='parameters', data=df,
                   fit_reg=False, scatter_kws={'alpha': .3}, height=1.5,
                   legend=False, row_order=reversed(qs))
    g.set(xscale='log', yscale='log', xlim=limx, ylim=limy)
    for i in range(len(ps)):
        for j in range(len(qs)):
            g.axes[i, j].plot(limx, limy, ls='--', c='black', lw=.5, zorder=0,
                              markeredgewidth=.1)
            g.axes[i, j].set_title(f'p={ps[j]}\nq={list(reversed(qs))[i]}',
                                   x=.05, y=.7, size='x-small', ha='left')
    g.set_axis_labels('', '')
    g.fig.text(0.45, .02, s='GCtree likelihood', multialignment='center')
    g.fig.text(.02, 0.7, s=f'frequency among {n} simulations', rotation=90,
    ...
    plt.figure()
    g = sns.lmplot(x='$\\hat{p}$', y='$\\hat{q}$', hue='true parameters',
                   data=df, fit_reg=False, scatter_kws={'alpha': .2}, height=6,
                   legend=False)
    g.set(xlim=(0.05, .45), xticks=np.arange(0., .6, .1), ylim=(.1, .9),
...
def infer(args):
    ...
    if args.bootstrap_phylipfile is not None:
        outfiles.extend(pp.parse_outfile(args.bootstrap_phylipfile,
                                         args.countfile, args.naive))
    ...
    for i, content in enumerate(outfiles):
        ...
        for tree in phylip_collapsed:
            if sum(tree.compare(tree2, method='identity')
        ...
        print('number of trees with integer branch lengths:',
              parsimony_forest.n_trees)

        # check for unifurcations at root
        unifurcations = sum(tree.tree.frequency == 0
                            and len(tree.tree.children) == 1
                            for tree in parsimony_forest.forest)
        if unifurcations:
            print(f'{unifurcations} trees exhibit unobserved unifurcation from'
        ...
        for tries in range(max_tries):
            try:
                ...
                if tries + 1 < max_tries:
                    print(f'floating point error in MLE: {e}. '
        ...
        if args.colormapfile is not None:
            with open(args.colormapfile, 'r') as f:
                ...
                for line in f:
                    ...
                    if ',' in color:
                        colors = {x.split(':')[0]: int(x.split(':')[1])
        ...
        for j, (l, collapsed_tree) in enumerate(
                                        zip(ls, parsimony_forest.forest), 1):
    ...
    if bootstrap:
        ...
        plt.figure()
        np.seterr(all='ignore')
        sns.jointplot('$\\hat{p}$', '$\\hat{q}$', data=df,
                      joint_kws={'alpha': .1, 's': .1},
                      space=0, color='k', stat_func=None, xlim=(0, 1),
                      ylim=(0, 1), height=3)
        ...
        gctrees[0].render(args.outbase
                          + '.inference.bootstrap_compatibility.svg',
                          colormap=colormap,
                          idlabel=args.idlabel,
                          chain_split=args.chain_split,
                          show_support=True)
...
def simulate(args):
    ...
    if args.sequence2 is not None:
        ...
        if args.frame is not None and args.frame != 1:
            ...
            args.sequence = args.sequence[
                (args.frame - 1):
                (args.frame - 1 + (3 * (((len(args.sequence)
                                          - (args.frame - 1)) // 3))))]
        ...
        seq_bounds = ((0, len(args.sequence)),
                      (len(args.sequence),
    ...
    if args.selection:
        if args.frame is None:
            raise Exception('Frame must be defined when simulating with '
                            'selection.')
        ...
        targetAAseqs = [mutation_model.one_mutant(args.sequence,
                                                  args.target_dist,
                                                  frame=args.frame)
        ...
        A_total = su.find_A_total(args.carry_cap, args.B_total, args.f_full,
                                  args.mature_affy, args.U)
        # Calculate the parameters for the logistic function:
        Lp = su.find_Lp(args.f_full, args.U)
        selection_params = [args.stop_dist, args.mature_affy, args.naive_affy,
                            args.target_dist, args.skip_update, targetAAseqs,
                            A_total, args.B_total, Lp, args.k, args.outbase]
    else:
    ...
    frequency, distance_from_naive, degree = zip(
            *[(node.frequency,
               ...
               sum(utils.hamming_distance(node.sequence, node2.sequence) == 1
    ...
    print(f'{sum(leaf.frequency for leaf in collapsed_tree.tree.traverse())}'
    ...
    if args.plotAA and args.selection:
        colormap = {node.name: colors[node.AAseq]
                    for node in collapsed_tree.tree.traverse()}
    else:
        colormap = {node.name: colors[node.sequence]
                    for node in collapsed_tree.tree.traverse()}
    ...
    if args.selection:
        ...
        palette = ['crimson', 'purple', 'hotpink', 'limegreen', 'darkorange',
                   'darkkhaki', 'brown', 'lightsalmon', 'darkgreen',
                   'darkseagreen', 'darkslateblue', 'teal', 'olive', 'wheat',
                   'magenta', 'lightsteelblue', 'plum', 'gold']
        # circular iterator
        ...
        colormap = {node.name: colors[node.target_dist]
                    for node in collapsed_tree.tree.traverse()}
        collapsed_tree.write(args.outbase
                             + '.simulation.collapsed_runstat_color_tree.p')
        collapsed_tree.render(args.outbase
                              + '.simulation.collapsed_runstat_color_tree.svg',
                              idlabel=args.idlabel,
                              colormap=colormap)
...
def get_parser():
    ...
    parser_infer.add_argument('--naive', type=str, default=None,
                              help='name of naive sequence (outgroup root)')
    ...
    parser_sim.add_argument(
        '--plotAA', type=bool, default=False,
    ...
    for subparser in [parser_test, parser_infer, parser_sim]:
        subparser.add_argument(
            '--outbase', type=str, default='gctree.out',
            help='output file base name')

    # common parameters for the inference and simulation subprograms
    for subparser in [parser_infer, parser_sim]:
        subparser.add_argument('--frame', type=int, default=None,
                               choices=(1, 2, 3), help='codon frame')
        subparser.add_argument(
            '--idlabel', action='store_true',
