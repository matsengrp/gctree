...
class LeavesAndClades():
    ...
    def logf(c, m, *params):
        ...
        if c == m == 0 or (c == 0 and m == 1):
            ...
            if m >= 1:
                neighbor_logf, (neighbor_dlogfdp,
                                neighbor_dlogfdq) = LeavesAndClades.logf(
                                                        c,
                                                        m - 1,
                                                        *params)
            ...
            for cx in range(c+1):
                for mx in range(m + 1):
                    if ((not (cx == mx == 0)) and (not (cx == c and mx == m))):
                        (neighbor1_logf,
                         (neighbor1_dlogfdp,
                          neighbor1_dlogfdq)) = LeavesAndClades.logf(cx, mx,
                                                                     *params)
                        (neighbor2_logf,
                         (neighbor2_dlogfdp,
                          neighbor2_dlogfdq)) = LeavesAndClades.logf(
                                                    c - cx, m - mx, *params)
            ...
            dlogfdp_result = np.multiply(softmax_logg_array,
                                         dloggdp_array).sum()
            dlogfdq_result = np.multiply(softmax_logg_array,
                                         dloggdq_array).sum()
...
class CollapsedTree(LeavesAndClades):
    ...
    def __init__(self, params=None, tree=None, frame=None, collapse_syn=False,
        ...
        if collapse_syn is True:
            ...
            for node in tree.iter_descendants():
                aa = Seq(node.sequence[(frame-1):
                                       (frame - 1 + (3 * (((len(node.sequence)
                                                            - (frame - 1)) // 3
                                                           ))))]
                         ).translate()
                aa_parent = Seq(node.up.sequence[
                                (frame - 1):
                                (frame - 1 + (3 * (((len(node.sequence)
                                                     - (frame - 1)) // 3))))]
                                ).translate()
        ...
        if tree is not None:
            ...
            for node in self.tree.traverse():
                if node.frequency > 0 or node == self.tree:
                    for name in ((node.name,) if isinstance(node.name, str)
                                 else node.name):
            ...
            assert (sum(node.frequency for node in tree.traverse())
                    == sum(node.frequency for node in self.tree.traverse()))

            rep_seq = (sum(node.frequency > 0 for node in self.tree.traverse())
                       - len(set([node.sequence
                                  for node in self.tree.traverse()
                                  if node.frequency > 0])))
    ...
    def mle(self, **kwargs):
        ...
        kwargs['sign'] = -1
        grad_check = check_grad(lambda x: self.ll(x, **kwargs)[0],
                                lambda x: self.ll(x, **kwargs)[1],
                                x_0)
        if grad_check > 1e-3:
            warnings.warn('gradient mismatches finite difference '
                          f'approximation by {grad_check}', RuntimeWarning)
        result = minimize(lambda x: self.ll(x, **kwargs), jac=True, x0=x_0,
    ...
    def simulate(self):
        ...
        for _ in range(self.m):
            # ooooh, recursion
            child = CollapsedTree(
                        params=self.params, frame=self.frame).simulate().tree
            child.dist = 1
    ...
    def render(self, outfile, idlabel=False, colormap=None, show_support=False,
               chain_split=None):
        ...
        def my_layout(node):
            ...
            if isinstance(circle_color, str):
                ...
                P = ete3.PieChartFace(
                        [100 * x / node.frequency
                         for x in circle_color.values()],
                ...
                T.hz_align = 1
                T.rotation = -90
                ete3.faces.add_face_to_node(P, node, 0,
                                            position='branch-right')
                ete3.faces.add_face_to_node(T, node, 1,
                                            position='branch-right')
            if idlabel:
        ...
        if idlabel:
            ...
            for node in self.tree.traverse():
                aln.append(SeqRecord(
                            Seq(str(node.sequence)), id=str(node.name),
    ...
    def compare(self, tree2, method='identity'):
        ...
        if method == 'identity':
            ...
            taxa = [node.sequence for node in self.tree.traverse()
                    if node.frequency]
            n_taxa = len(taxa)
            ...
            for i in range(n_taxa):
                nodei_true = self.tree.iter_search_nodes(
                                        sequence=taxa[i]).__next__()
                nodei = tree2.tree.iter_search_nodes(
                                    sequence=taxa[i]).__next__()
            ...
            try:
                return tree1_copy.robinson_foulds(
                    tree2_copy, attr_t1='sequence', attr_t2='sequence',
                    unrooted_trees=True)[0]
            except:
                return tree1_copy.robinson_foulds(
                        tree2_copy, attr_t1='sequence', attr_t2='sequence',
                        unrooted_trees=True, allow_dup=True)[0]
        else:
    ...
    def get_split(self, node):
        ...
        assert taxa1.union(taxa2) == set((name
                                          for node in self.tree.traverse()
                                          if node.frequency > 0
                                          or node == self.tree
        ...
        for node in self.tree.get_descendants():
            ...
            for i, tree in enumerate(bootstrap_trees_list):
                ...
                for boot_node in tree.tree.get_descendants():
                    ...
                    if (compatibility
                            and compatible
                            and not self.split_compatibility(split,
                                                             boot_split)):
                        compatible = False
                    if (not compatibility and not supported
                            and boot_split == split):
                        supported = True
                if supported:
...
class CollapsedForest(CollapsedTree):
    ...
    def __init__(self, params=None, n_trees=None, forest=None):
        ...
        if forest is None and params is None:
            raise ValueError('either params or forest (or both) must be '
                             'provided')
    ...
    def simulate(self):
        ...
        self.forest = [CollapsedTree(self.params).simulate()
    ...
    def ll(self, params, sign=1, empirical_bayes_sum=False, build_cache=True):
        ...
        if self.forest is None:
            raise ValueError(
                'forest data must be defined to compute likelihood')
        if sign not in (-1, 1):
            raise ValueError('sign must be 1 or -1')
        if build_cache:
            c_max = max(node.frequency
                        for tree in self.forest
                        for node in tree.tree.traverse())
            m_max = max(len(node.children)
                        for tree in self.forest
                        for node in tree.tree.traverse())
            LeavesAndClades.build_logf_cache(c_max, m_max, *params)
        ...
        if empirical_bayes_sum:
            ...
            return sign*(-np.log(len(ls))
                         + logsumexp(ls)), sign*np.array(grad_l)
        else:
