...
df = pd.melt(
    aggdat,
    id_vars=["method", "N_taxa", "lambda", "lambda0"],
    value_vars=["RF", "MRCA"],
    var_name="metric",
)
df.loc[df.loc[:, "metric"] == "MRCA", "value"] = np.log(
    1 + df.loc[df.loc[:, "metric"] == "MRCA", "value"]
)
...
numb_variables = (
    sum(len(set(df[var])) > 1 for var in variables) - 1
)  ### NOTICE -1 because of N_taxa cannot be plotted on same page because of "tight layout"
