...
"""aggregation plots of validation output from several simulation/validation
runs."""
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="use validation.tsv files containing other metrics",
)
...
aggdat = pd.DataFrame(
    columns=(
        "simulations ranked by parsimony degeneracy",
        "parsimony forest size",
        "MRCA distance to true tree",
        "RF distance to true tree",
        "COAR distance to true tree",
        "COAR_fw distance to true tree",
        "log-likelihood",
        "ismle",
    )
)
...
for fname in args.input:
    df = pd.read_csv(
        fname, sep="\t", usecols=("COAR", "COAR_fw", "MRCA", "RF", "log-likelihood")
    )
    frames.append(df)
sorted_indices = sorted(
    range(len(frames)), key=lambda i: len(frames[i].index), reverse=True
)
...
if args.allmetrics:
    ...
    for fname in [f.replace(".gctree", "") for f in args.input]:
        df = pd.read_csv(
            fname, sep="\t", usecols=("method", "COAR", "COAR_fw", "MRCA", "RF")
        )
    ...
    for i, metric in enumerate(
        ...
        if not args.allmetrics:
            sns.boxplot(
                ...
                y=metric,
                data=aggdat[aggdat["ismle"] == False],
                color="gray",
            )
            sns.stripplot(
                x="simulations ranked by parsimony degeneracy",
            ...
            sns.stripplot(
                ...
                jitter=2,
                color=next(palette),
                data=aggdat_other[aggdat_other["method"] == "gctree"],
            )
            sns.stripplot(
                x="simulations ranked by parsimony degeneracy",
                y=metric,
    ...
    if args.allmetrics:
        ...
        for metric in (
            ...
            for i in range(nmethods):
                for j in range(i + 1, nmethods):
                    p = mannwhitneyu(
                        aggdat_other[metric][aggdat_other["method"] == methods[i]],
                        aggdat_other[metric][aggdat_other["method"] == methods[j]],
                        alternative="two-sided",
                    )[1]
                    print(
                        "U test significance ({}, {} vs {}): {}".format(
                            metric, methods[i], methods[j], p
                        )
                    )
                p = mannwhitneyu(
                    aggdat_other[metric][aggdat_other["method"] == methods[i]],
                ...
                print(
                    "U test significance ({}, {} vs parsimony): {}".format(
                        metric, methods[i], p
                    )
                )
        ...
        for metric in (
            ...
            if len(set(aggdat[metric])) > 1:
                p = mannwhitneyu(
                    aggdat[metric][aggdat["ismle"] == True],
