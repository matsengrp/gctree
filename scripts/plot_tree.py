...
def read_plot_config(fnam):
    ...
    if not_found:
        raise RuntimeError(
            "Key not found in plot config: [{0}]\nPossible keys: [{1}]".format(
                ", ".join(str(i) for i in not_found),
                ", ".join(str(i) for i in allowed_keys),
            )
        )
...
def tree_render_user(tree, frame=None, tree_features=None, namecolor=None):
    ...
    for node in tree.traverse():
        ...
        if not no_default and node.frequency == 0 and not hasattr(node, "cl"):
            ...
            # Assign node size:
            if "node_size" in tree_features and ifhasthenget(
                node, tree_features["node_size"]
            ):
                nstyle["size"] = (
                    small_node * scipy.sqrt(getattr(node, tree_features["node_size"]))
                    + small_node
                )
                nstyle[
                    "fgcolor"
                ] = "black"  # Keep default color unless otherwise is specified below
            ...
            for t in tree_features["node_label"]:
                ...
                if node_attr and to_string(node_attr):
                    ...
                    if "e-" in textface and to_float(
                        ...
                        N = TextFace(
                            "%2.1e    " % to_float(textface), fsize=12, fgcolor="black"
                        )

        # Add default values:
        elif not no_default and node.frequency > 1 and not hasattr(node, "cl"):
            N = TextFace(
                node.frequency, fsize=12, fgcolor="black"
            )  # Default: use node frequency as TextFace
...
def update_after_collapse(node_up, node, tree_features=None):
    ...
    for feature in feature_set:
        if hasattr(node, feature) and not hasattr(node_up, feature):
            ...
            hasattr(node, feature)
            and hasattr(node_up, feature)
            and getattr(node_up, feature) == ""
        ):
            setattr(node_up, feature, getattr(node, feature))
        elif (
            hasattr(node, feature)
            and hasattr(node_up, feature)
            and getattr(node, feature) == ""
        ):
            pass
        elif hasattr(node, feature) and hasattr(node_up, feature):
            if (
                is_number(getattr(node, feature))
                and feature in tree_features["collapse_syn"]
            ):
                if tree_features["collapse_syn"][feature] == "sum":
                    setattr(
                        node_up,
                        feature,
                        (getattr(node, feature) + getattr(node_up, feature)),
                    )
                elif tree_features["collapse_syn"][feature] == "mean":
                    setattr(
                        node_up,
                        feature,
                        (getattr(node, feature) + getattr(node_up, feature)) / 2,
                    )
                else:
                    raise Exception(
                        "Do not know this option:",
                        tree_features["collapse_syn"][feature],
                    )
...
def collapse_syn(tree, frame=None, tree_features=None):
    ...
    for node in tree.iter_descendants():
        aa = Seq(
            node.sequence[
                (frame - 1) : (
                    frame - 1 + (3 * (((len(node.sequence) - (frame - 1)) // 3)))
                )
            ]
        ).translate()
        aa_parent = Seq(
...
def make_tree(
    tree, outfile, tree_features_file=None, statsfile=None, frame=None, namecolor=None
):
    try:
    ...
    if "prune_max_branch_length" in tree_features and to_float(
        tree_features["prune_max_branch_length"]
    ):
        tree = prune_long_branches(
            tree, to_float(tree_features["prune_max_branch_length"])
        )
    ...
    if frame is not None and "collapse_syn" in tree_features:
        ...
        raise Exception(
            "Cannot collapse synonymous mutations when frame parameter is not specified."
        )
    ...
    if "collapse_low_freq" in tree_features:
        try:
            ...
            raise RuntimeError(
                "Could not cast f and k to integers, or maybe they are not defined at all?:",
                e,
            )
        tree = collapse_low_freq(tree, [f, k], cl_except)

    tree, ts = tree_render_user(
        tree, frame=frame, tree_features=tree_features, namecolor=namecolor
    )
    tree.render(outfile, tree_style=ts)
...
def main():
    ...
    sys.path.append(
        os.path.abspath("/".join(os.path.realpath(__file__).split("/")[:-2]) + "/bin")
    )
    import argparse
    import pickle
    ...
    parser.add_argument(
        ...
        dest="tree_numb",
        metavar="INT",
        help="Index specifying which tree to pick from the forrest.",
    )
    parser.add_argument(
        "--outfile",
        required=True,
        ...
        dest="outfile",
        metavar="FILENAME",
        help="Output tree filename.",
    )
    parser.add_argument(
        "--frame",
        type=int,
        dest="frame",
        help="Which frame is the node sequences in? If unspecified frame is not enforced and stop codons are allowed.",
    )
    parser.add_argument(
        "--config",
        type=str,
        dest="config",
        metavar="JSON",
        help="JSON formatted file that specifies which features to plot and how.",
    )
    parser.add_argument(
        "--statsfile",
        type=str,
        dest="statsfile",
        help="Comman separated list of features linked to the node name.",
    )
    ...
    collapsed_tree = CollapsedTree(
        tree=forest.forest[args.tree_numb].tree, frame=args.frame
    )
    make_tree(
        collapsed_tree.tree,
        args.outfile,
        tree_features_file=args.config,
