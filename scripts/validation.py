...
def lineage_dist(
    ...
    for node in true_tree.tree.iter_leaves():  # Iterate only through the leaves
        ...
        aln_res = align_lineages(
            ...
            allow_double_gap=allow_double_gap,
        )
    ...
    if (
        len(norm_lineage_dist) == 0
    ):  # There can be total_max_penalty == 0 when all lineages have less than three members
        return 0
...
def validate(true_tree, inferences, true_tree_colormap, outbase):
    ...
    if "gctree" in inferences:
        ...
        distances, likelihoods = zip(
            *[
                (
                    true_tree.compare(tree, method="RF"),
                    tree.ll(inferences["gctree"].params)[0],
                )
                for tree in inferences["gctree"].forest
            ]
        )
        MRCAs = [
            true_tree.compare(tree, method="MRCA")
            for tree in inferences["gctree"].forest
        ]
        lineage_distances = [
            all_lineage_dist(true_tree, tree) for tree in inferences["gctree"].forest
        ]
        lineage_distances = list(
        ...
        mean_branch_lengths = [
            np.mean([node.dist for node in tree.tree.iter_descendants()])
            for tree in inferences["gctree"].forest
        ]
        df = pd.DataFrame(
            {
                "log-likelihood": likelihoods,
                "RF": distances,
                "MRCA": MRCAs,
        ...
        if n_trees > 1:
            ...
            for i, metric in enumerate(("RF", "MRCA", "COAR", "COAR_fw"), 1):
                ...
                ax = sns.regplot(
                    "log-likelihood",
                    metric,
                    data=df[df["log-likelihood"] != maxll],
                    fit_reg=fit_reg,
                    color="black",
                    scatter_kws={"alpha": 0.8, "clip_on": False},
                )
                sns.regplot(
                    "log-likelihood",
                    metric,
    ...
    methods, n_taxa, distances, MRCAs, lineage_distances = zip(
        *[
            (
                ...
                all_lineage_dist(true_tree, inferences[method].forest[0]),
            )
    ...
    lineage_distances = list(
        zip(*lineage_distances)
    )  # Unzip the methods tuple to get lineage_distances[ld0-3][method]
    df = pd.DataFrame(
        {
            "method": methods,
            "N_taxa": n_taxa,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="validate results of inference on simulation data",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )

    parser.add_argument("true_tree", type=str, help=".p file containing true tree")
    parser.add_argument(
        "true_tree_colormap", type=str, help=".tsv colormap file for true tree"
    )
    parser.add_argument(
        "forest_files",
        type=str,
        nargs="*",
        help=".p files containing forests from each inference method",
    )
    ...
    parser.add_argument(
        "--outbase", type=str, required=True, help="output file base name"
    )
    global args
    ...
    with open(args.true_tree_colormap, "r") as f:
        for line in f:
            ...
            if "," not in name:
                true_tree_colormap[
                    true_tree.tree.search_nodes(name=name)[0].sequence
                ] = color
            else:
