...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="compare heavy and light chain trees",
    ...
    heavy_log = pd.read_csv(
        args.heavy_log, sep="\t", usecols=("logLikelihood",), squeeze=True
    )
    light_log = pd.read_csv(
        args.light_log, sep="\t", usecols=("logLikelihood",), squeeze=True
    )
    ...
    for i, heavy_tree in enumerate(heavy_forest.forest):
        for j, light_tree in enumerate(light_forest.forest):
            ...
            dat.loc[ct] = [
                ...
                heavy_tree.tree.robinson_foulds(
                    light_tree.tree, attr_t1="name", attr_t2="name", unrooted_trees=True
                )[0],
                "both"
                if i == j == 0
                else "heavy"
    ...
    sns.boxplot(
        x="chain", y="logLikelihood", data=dat2[dat2["mle"] == False], color="gray"
    )
    ax = sns.stripplot(
        x="chain", y="logLikelihood", data=dat2[dat2["mle"] == True], color="red"
    )
    ...
    plt.hist(
        (
            dat["RF"][dat["mle"] == "both"],
            dat["RF"][dat["mle"] == "heavy"],
            dat["RF"][dat["mle"] == "light"],
            dat["RF"][dat["mle"] == "neither"],
        ),
        bins=scipy.arange(-0.5, max(dat["RF"]) + 1.5, 1.0),
        histtype="barstacked",
        label=("IgH and IgL MLE", "IgH MLE", "IgL MLE"),
        color=("red", "indianred", "rosybrown", "gray"),
        edgecolor="black",
    )
    plt.legend(loc=(0, 0.68))
    plt.xlim([min(dat["RF"]) - 0.5, max(dat["RF"]) + 0.5])
    ...
    for j, heavy_tree in enumerate(heavy_forest.forest):
        for i, light_tree in enumerate(light_forest.forest):
            dat[i, j] = heavy_tree.tree.robinson_foulds(
                light_tree.tree, attr_t1="name", attr_t2="name", unrooted_trees=True
            )[0]
    ...
    plt.figure(figsize=(0.3 * len(heavy_forest.forest), 0.3 * len(light_forest.forest)))
